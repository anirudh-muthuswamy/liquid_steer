...
# image frames and the corresponding steering angle for the frtame, allowing for data loading and transformation.
#  x -> Input tensor of shape (C, seq_len, H, W).
#  y -> Target tensor of shape (seq_len,1) containing the steering angle for the 'seq_len' frames.

#there is a step size argument that is used to step through 'step_size' number of images before going to the 
# next sequence. This is allowed for downsizing and test different sample sizes for training
